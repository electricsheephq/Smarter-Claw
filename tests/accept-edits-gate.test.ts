/**
 * Ported from openclaw-1: src/agents/plan-mode/accept-edits-gate.test.ts
 *
...
describe("checkAcceptEditsConstraint — self-restart (blocked)", () => {
  ...
  it("blocks pipe-chained `pgrep openclaw | xargs kill`", () => {
    const r = checkAcceptEditsConstraint({
      toolName: "exec",
  ...
  it("blocks `kill $(pgrep openclaw)` subshell form", () => {
    expect(
      checkAcceptEditsConstraint({
        toolName: "exec",
  ...
  it("blocks backtick form `kill `pgrep gateway``", () => {
    expect(
      checkAcceptEditsConstraint({
        toolName: "exec",
...
describe("checkAcceptEditsConstraint — config change (blocked)", () => {
  ...
  it("blocks absolute $HOME form that expands to `~/.openclaw/`", () => {
  ...
  it("blocks `..` traversal that resolves into `~/.openclaw/`", () => {
    const r = checkAcceptEditsConstraint({
      toolName: "write",
  ...
  it("blocks multi-segment traversal back into `~/.openclaw/`", () => {
    const r = checkAcceptEditsConstraint({
      toolName: "edit",
...
describe("checkAcceptEditsConstraint — shell-escape layered defense", () => {
...
describe("extractApplyPatchTargetPaths — `*** Move to:` grammar", () => {
  it("extracts destination from `*** Move to:` inside an `*** Update File:` hunk", () => {
    ...
    expect(paths).toContain("src/old/name.ts");
