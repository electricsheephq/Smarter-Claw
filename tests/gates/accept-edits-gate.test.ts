 ...
 *
 * Ported from in-host
 * `/Users/lume/repos/openclaw-pr70071-rebase/src/agents/plan-mode/accept-edits-gate.test.ts`
 * (commit `ea04ea52c7`). Per the plan's parity contract, every test
 * case here mirrors an in-host assertion. The gate blocks three
 * classes of action when `acceptEdits` permission is active:
 * destructive, self-restart, and config-change. This test suite
 * exercises positive cases (legit tool use passes) and negative
...
} from "../../src/gates/accept-edits-gate.js";
