/**
 * Ported from openclaw-1: src/agents/plan-mode/plan-archetype-persist.test.ts
 */
...
} from "../src/archetype-persist.js";

describe("persistPlanArchetypeMarkdown", () => {
  let tmpBase: string;
  const FIXED_DATE = new Date("2026-04-18T15:30:00Z");

  beforeEach(async () => {
    tmpBase = await fs.mkdtemp(path.join(os.tmpdir(), "smarter-claw-plan-persist-"));
  });
  ...
  describe("recoverable storage errors", () => {
    ...
    it("EEXIST collision path still loops and eventually reports the cap", async () => {
      await expect(
        persistPlanArchetypeMarkdown({
          agentId: "main",
