/**
 * Plan-mode reference card — bootstrap-injected, persistent reference an
 * in-mode agent sees alongside the decision-completeness archetype prompt.
 *
 * Ported verbatim from openclaw-1 (commit a34b2a1ce2). Pure data — no
 * imports, no host dependencies. The full original rationale was:
 *
 *   Without this, a fresh agent has a 2-turn learning curve before it
 *   sees the in-mode system prompt. The reference card collapses that
 *   to ZERO turns: as soon as plan mode is active, the agent sees the
 *   diagram, contract, tag taxonomy, and pitfalls — all on the SAME
 *   turn the runtime decides plan mode is active.
 *
 ...
 *
 * Companion artifact: the same content (in markdown form) ships as the
 * `plan-mode-101` skill so an agent in NORMAL mode that's asked
 * "explain plan mode" can invoke the skill on demand. Keep both
 * surfaces in sync — when this file changes, update the SKILL.md too.
 */
export const PLAN_MODE_REFERENCE_CARD = [
  ...
  "## Debugging tips",
  "",
  "- Turn on plan-mode debug logging: `openclaw config set plugins.entries.smarter-claw.config.debugLog true` then restart gateway.",
  "- Tail the structured event log: `tail -F ~/.openclaw/logs/gateway.err.log | grep '\\[smarter-claw/'`",
