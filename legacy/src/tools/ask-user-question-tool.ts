/**
 * `ask_user_question` tool — surfaces a clarifying question to the user
 * via the same approval-card pipeline that exit_plan_mode uses
 * (kind: "plugin"). The user picks one of N options (or types free
 * text when allowed), and the answer arrives in the next agent turn
 * as a synthetic user message tagged `[QUESTION_ANSWER]`.
 *
 ...
 * buttons in the Control UI and Telegram, and as a `/plan answer
 * <choice>` text command on plain channels.
 */
