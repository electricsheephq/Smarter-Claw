...
import { buildPlanFilename } from "./archetype-prompt.js";
...
export interface PersistPlanArchetypeMarkdownInput {
  ...
  baseDir?: string;
  /**
   * Test-only hook: override the writeFile function used for the final
   * markdown write. Lets tests inject ENOSPC/EACCES/EIO without having
   * to mock the ESM module namespace. Production never sets it; omit
   * → uses `fsp.writeFile` directly.
   */
  _writeFileForTest?: (
    path: string,
    data: string,
...
export async function persistPlanArchetypeMarkdown(
  ...
  //
  // Also reject "." / ".." / any agentId composed entirely of dots —
  // `path.join(baseDir, "..", "plans")` would escape the intended
  // directory. Additionally verify the resolved target stays within
  // baseDir as a belt-and-suspenders prefix check.
  ...
  // enumerate).
  //
  // Also reject symlinks at the agent-dir and plans-dir levels, then
  // validate containment using realpath() (not just lexical resolve()).
  // A pre-existing symlink like `~/.openclaw/agents/<agentId> -> /etc`
  // would let writes escape baseDir despite the syntactic agentId
  // check (the path component `<agentId>` is fine; the symlink target
  // is the escape vector).
  ...
  //
  // Atomic create with `wx` (exclusive) flag instead of `existsSync` +
  // `writeFile`. The existsSync check is a TOCTOU race window — a
  // parallel agent call writing the same date+slug could land between
  // the existence check and our write, silently overwriting their plan.
  // `wx` opens with `O_CREAT | O_EXCL`, so the OS rejects the open with
  // EEXIST when the file already exists. We catch EEXIST and try the
  // next suffix in the same loop. All other errors propagate.
  ...
  while (n <= MAX_COLLISION_SUFFIX) {
    try {
      ...
      // Classify recoverable system-admin errors with a distinctive
      // prefix so the caller's catch can surface an actionable message
      // instead of a generic "persist failed". ENOSPC = disk full,
      // EACCES = permissions, EIO = underlying storage I/O error. All
      // are remediable by the operator, not by retrying the agent
      // turn.
 ...
 * failure) surface as this class so the bridge can emit an actionable
 * operator-facing log message without confusing the path with a
 * genuine bug. Plan-mode treats these as non-fatal — the plan
 * approval still proceeds; only the durable audit artifact is lost.
 */
