/**
 * Plan-archetype steering — appended to the system prompt when the
 * session is in plan mode so the agent produces decision-complete
 * plans (Opus-quality) instead of a few paragraphs + checklist.
 *
 * Adapted from the Plan Mode example prompt and tightened for
...
/**
 * Build a kebab-case filename slug from a plan title.
...
/**
 * Build the canonical plan filename. ISO date prefix ensures
