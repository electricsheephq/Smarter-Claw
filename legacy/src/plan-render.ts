 ...
 * activeForm) PLUS closure-gate fields (acceptanceCriteria,
 * verifiedCriteria). The status union here includes `cancelled`.
...
export interface PlanStepForRender {
  ...
  activeForm?: string;
  /**
   * Closure-gate fields. Optional; when present, the channel renderer
   * surfaces acceptance criteria as a nested checklist under each step
   * (verified vs not). Backwards-compatible: omit both fields and
...
function renderStepLine(
  ...
  switch (format) {
    case "html": {
      // Neutralize mentions BEFORE escaping so an agent-controlled step
      // text like "@everyone deploy now" can't ping a Telegram channel
      // or any HTML-rendering surface that follows mention conventions.
    ...
    case "markdown": {
      // Markdown renders on Discord, Mattermost, Matrix, MSTeams,
      // GoogleChat, Feishu, web, CLI. Without neutralization, an
      // agent-controlled step text containing "@everyone" pings the
      // entire channel on Discord + Mattermost. escapeMarkdown handles
      // `*`/`[`/etc but does NOT touch `@`, so we need a separate pass.
    ...
    case "slack-mrkdwn": {
      // Apply mention-neutralization the same way the html / markdown /
      // plaintext branches do, BEFORE the format-specific escape. This
      // lets `escapeSlackMrkdwn` drop its indiscriminate `@`
      // replacement (which would mangle emails like `user@example.com`)
...
/**
 * Render a step's acceptance criteria as a nested checklist beneath the
 * step line. Indentation + marker style matches the parent format's
 * conventions. Returns `[]` (no lines) when no criteria are declared so
 * existing simple plans render unchanged.
 *
 * Each criterion uses the same content sanitization as the parent step
 * label so injection vectors are equivalent across both layers.
 */
function renderAcceptanceCriteria(step: PlanStepForRender, format: PlanRenderFormat): string[] {
  ...
  // Normalize the verified set the same way criteria are normalized
  // before comparison. Without this, a criterion verified upstream with
  // equivalent text differing only by whitespace/newline formatting
  // would render as unchecked here.
...
/**
 * Render the full plan archetype as a single markdown document suitable
 * for persistence to disk and delivery as a file attachment (e.g., a
...
export function renderFullPlanArchetypeMarkdown(input: PlanArchetypeMarkdownInput): string {
  ...
  if (input.analysis && input.analysis.trim()) {
    // Analysis is multi-paragraph. Preserve paragraph breaks but strip
    // carriage returns + escape per-line. Newlines in markdown ARE
    // meaningful, so we preserve `\n\n` as paragraph separators.
  ...
  // Plan section is REQUIRED — but if the steps array is empty, emit a
  // placeholder note rather than an empty section header.
  lines.push("", "## Plan");
  ...
  // the file (Telegram primarily, but Discord/Slack mirror the same
  // pattern via universal slash commands).
 ...
 *
 * `~` is in the escape set so that step text containing `~~` doesn't
 * close the outer strikethrough wrapper used for cancelled steps.
 */
 ...
 *
 * Also neutralize Discord raw user mentions `<@123>` / `<@!123>` /
 * `<@&123>` (role mention) by inserting U+200B between `<` and `@`.
 * Discord parses these as pings; an agent embedding such a string in
 * a plan step would otherwise notify the named user/role on
 * `/plan restate`.
 *
...
/**
 * Bounded set with FIFO eviction to prevent unbounded growth in
 * long-running gateway processes if a malformed/malicious upstream
 * produces many distinct statuses over time. Once `WARNED_STATUSES_MAX`
 * is reached, the oldest tracked status is evicted on each new insert
 * (Set iteration order is insertion order in ES2015+, so the first key
 * is the oldest).
 */
