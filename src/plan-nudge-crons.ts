 ...
 * reads the persisted plan-mode state and either continues the plan or
 * — if the plan has already been resolved (completed, exited, or
 * rejected) — exits cleanly via the heartbeat pathway (the nudge
 * prefix returns null when no plan is active, so a resolved-plan
 * wake-up degrades to a normal heartbeat no-op).
 *
 ...
 * the cron-remove seam. Best-effort — if cron removal fails, the
 * nudges fire harmlessly into a normal-mode session and degrade to
 * no-op.
 *
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ INSTALLER SEAM:                                                 │
 * │                                                                 │
 * │ The Plugin SDK does not expose `cron.add` / `cron.remove`       │
 * │ today. Callers must inject a `callGatewayTool`-shaped function  │
 * │ via `deps.callGatewayTool`. The installer-side wiring binds     │
 * │ this to the host's gateway client (or the plugin SDK's          │
 * │ equivalent once the seam lands).                                │
 * │                                                                 │
 * │ Until the seam lands, calling these helpers without `deps`      │
 * │ will throw a clear error so operators notice the gap rather     │
 * │ than silently losing nudges.                                    │
 * └─────────────────────────────────────────────────────────────────┘
 */
 ...
 *
 * The escape hatch is the per-call `intervals` argument; there's no
 * static config knob today.
 */
...
export interface PlanNudgeSchedulerDeps {
   ...
   * Signature mirrors the openclaw-1 internal `callGatewayTool(method,
   * opts, params)` contract — this stays installer-side because the
   * Plugin SDK does not expose a cron seam yet. When omitted, the
   * scheduler throws (no silent no-op — operators need to notice the
   * gap).
   */
 ...
 * the plan-mode state for later cleanup.
 *
...
export async function schedulePlanNudges(params: {
  ...
  if (!call) {
    throw new Error(
      "schedulePlanNudges: deps.callGatewayTool is required (the Plugin SDK does not expose a cron seam yet — installer-side wiring must inject the gateway-tool client).",
    );
  }
  ...
  for (const minutes of intervals) {
    ...
    try {
      ...
      // reads the persisted plan-mode state from disk and figures out
      // which step to advance. If plan mode has already been exited
      // / completed by the time this fires, the heartbeat-runner's
      // `buildActivePlanNudge` returns null and the turn degrades to
      // standard heartbeat behavior (no-op).
      // `[PLAN_NUDGE]:` prefix matches the family of plan-mode
      // synthetic messages so channel renderers can identify them and
      // future code paths can hide them from user-visible chat.
      const message =
      ...
      // Validate sessionKey against the cron service's constraints (no
      // `/`, `\`, or `\0` characters). Keeps the failure local +
      // actionable instead of a generic cron-validator error 60
      // seconds later.
...
export async function cleanupPlanNudges(params: {
  ...
  if (!call) {
    throw new Error(
      "cleanupPlanNudges: deps.callGatewayTool is required (the Plugin SDK does not expose a cron seam yet — installer-side wiring must inject the gateway-tool client).",
    );
  }
  let removed = 0;
  let failed = 0;
