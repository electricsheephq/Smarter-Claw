 ...
 * `accept-edits-gate.ts` at
 * `/Users/lume/repos/openclaw-pr70071-rebase/src/agents/plan-mode/accept-edits-gate.ts`
 * (commit `ea04ea52c7`). The destructive-prefix list, SQL patterns,
 * find-flag patterns, escape-vector patterns, self-restart patterns,
 * config-change patterns, protected-config-path prefixes, and
 * path-writer tool set are ALL part of the security contract — diverging
 * from in-host risks either over-blocking (UX regression) or
 * under-blocking (security regression). Comments below preserved
 * verbatim from in-host because they document specific attack vectors
 * and Codex review #s.
 *
