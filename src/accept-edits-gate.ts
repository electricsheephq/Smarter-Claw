 ...
 *     `approval-state.ts` teaches the agent the three constraints and
 *     tells it to ask the user before invoking any of them.
 *
 ...
 * │ INSTALLER SEAM:                                                 │
 * │                                                                 │
 * │ Same activation pattern as `mutation-gate.ts` — the gate fires  │
 * │ at the host's `before_tool_call` hook (not yet in the public    │
 * │ Plugin SDK). The installer-side patch should:                   │
 * │                                                                 │
 * │   1. Read the session's plugin-namespaced state via              │
 * │      `readSmarterClawState(session)` and check                   │
 * │      `state?.postApprovalPermissions?.acceptEdits === true`.    │
 * │   2. Only when acceptEdits is granted, call                     │
 * │      `checkAcceptEditsConstraint({ toolName, execCommand,       │
 * │      filePath, additionalPaths })`.                             │
 * │   3. For `apply_patch` calls, also call                         │
 * │      `extractApplyPatchTargetPaths(input)` and pass the result  │
 * │      as `additionalPaths` so paths embedded in the patch        │
 * │      envelope are checked alongside the singular `filePath`.    │
 * │   4. When `result.blocked === true`, synthesize a tool-result   │
 * │      with `result.reason` as the visible body and return that   │
 * │      to the agent without ever executing the underlying tool.   │
 * └─────────────────────────────────────────────────────────────────┘
 */
...
export interface AcceptEditsGateParams {
  ...
  filePath?: string;
  /**
   * Additional paths extracted from tool inputs that carry MULTIPLE
   * target paths (specifically `apply_patch`, where the patch text in
   * `params.input` contains target paths in its envelope headers).
   * Each entry is checked against the protected-config-path prefixes
   * individually. Optional — if the caller can't parse out additional
   * paths, the single `filePath` field still works.
   */
...
/**
 * Layered-defense escape-pattern detection. The prefix / SQL / find
 * checks above catch the 99% case where the destructive verb is
 * directly visible in the command string. These patterns flag the
 * sophisticated-bypass vectors where a shell would resolve an
 * expansion AT RUNTIME into a destructive command — the gate can't
 * track the expansion, but it can refuse to allow the construct
 * entirely under acceptEdits.
 *
 * Posture: if an exec command contains ANY of these escape constructs
 * referencing destructive verbs, treat it as destructive and block.
 * Rationale:
 *   - acceptEdits is a permission elevation — the user opted in for
 *     trusted-plan execution, not for cleverness budget.
 *   - A legitimate post-approval exec rarely needs env-var indirection
 *     for destructive verbs. Blocking has near-zero false-positive
 *     cost and high true-positive recall.
 *   - Primary defense remains the prompt layer; this is defense-in-
 *     depth so a prompt-ignoring agent can't silently shell-escape
 *     around the gate.
 */
...
const DESTRUCTIVE_ESCAPE_PATTERNS: readonly RegExp[] = [
  // `$RM file`, `$SHRED ...` — env-var indirection where the variable
  // name matches a destructive verb (case-insensitive).
  ...
  // concatenation of adjacent quoted fragments that together spell a
  // destructive verb — catches the common "r""m" / "rm"+"" / "r"m
  // patterns. Intentionally conservative — matches when adjacent
  // quoted tokens start with the first letter of a destructive verb
  // and can reconstruct into it.
  /["'][a-z]["']["'][a-z]["']/i,
  // Hex-encoded destructive verbs: `\x72m`, `\x72\x6d`. A destructive
  // verb's first letter is `\xNN` followed by the remainder.
  // Conservative — also flags any `\xNN` byte escape inside an exec
  // command, which is itself highly suspicious under acceptEdits.
  /\\x[0-9a-f]{2}/i,
...
function checkDestructive(execCommand: string): AcceptEditsGateResult | null {
  ...
  // Layered-defense: catch escape-vector bypasses where the destructive
  // verb is hidden behind env expansion, subshell, quote concatenation,
  // or byte escapes.
 ...
 * `state.postApprovalPermissions.acceptEdits === true`. If acceptEdits
 * is not granted, this gate is not invoked at all.
 *
 * Returns `{ blocked: false }` for anything that doesn't match one of
 * the three constraint categories. Fail-open by design — this layer
 * exists to catch explicit destructive / restart / config actions, not
 * to restrict general mutation.
 */
export function checkAcceptEditsConstraint(params: AcceptEditsGateParams): AcceptEditsGateResult {
  ...
  if (PATH_WRITER_TOOLS.has(toolName)) {
    // Check EVERY candidate path (the singular `filePath` from
    // params.path / params.filePath / params.file_path PLUS any
    // additionalPaths the caller extracted from a multi-path input
    // like `apply_patch`'s patch envelope). Return the first
    // protected-path hit. Without this, only the singular `filePath`
    // would be checked, leaving `apply_patch` calls (which embed paths
    // in `params.input`) able to bypass the protected-path block.
...
/**
 * Parse target paths from an `apply_patch` envelope text.
 *
 * The patch format uses `*** Update File: <path>` / `*** Add File: <path>`
 * / `*** Delete File: <path>` headers. Returns all unique paths found;
 * returns an empty array if `input` is missing/non-string or no headers
 * match. Tolerant to whitespace and case variations on the verb token.
 *
 * Used by the before-tool-call hook to feed `additionalPaths` into
 * `checkAcceptEditsConstraint` so the protected-config-path block fires
 * for `apply_patch` calls under acceptEdits.
 */
export function extractApplyPatchTargetPaths(input: unknown): string[] {
  if (typeof input !== "string" || input.length === 0) {
    return [];
  }
  // Match the three single-path envelope verbs (Update/Add/Delete) and
  // the Move destination marker. The actual apply_patch grammar uses
  // `*** Move to: <dst>` as a SUB-marker nested inside an `*** Update
  // File: <src>` hunk — NOT the older `*** Move File: <src> -> <dst>`
  // single-line form. The source path is already caught by
  // `singlePathRe` (the surrounding `*** Update File:` line); the
  // `moveToRe` catches the destination.
