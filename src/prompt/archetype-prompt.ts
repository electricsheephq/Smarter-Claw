/**
 * Plan-mode archetype system-prompt fragment.
 *
 * **Parity contract**: byte-identical port of the in-host
 * `PLAN_ARCHETYPE_PROMPT` at
 * `/Users/lume/repos/openclaw-pr70071-rebase/src/agents/plan-mode/plan-archetype-prompt.ts`
 * (commit `ea04ea52c7`, PR-10 of the in-host ladder).
 *
 * # The string is the contract
 *
 * The agent reads this prompt verbatim. Bytes matter:
 * - Prompt-caching: prefix bytes hash into the cache key. A single
 *   character drift bumps the hash and forces a full prefix re-evaluation
 *   on every plan-mode turn.
 * - Steering quality: the wording was tuned through the in-host's
 *   PR-10 development cycle. Paraphrases regress agent behavior.
 *
 * Tests assert byte-identical match against the in-host source
 * (parity-harness Layer 1 extension at
 * tests/parity/archetype-prompt-parity.test.ts).
 */
