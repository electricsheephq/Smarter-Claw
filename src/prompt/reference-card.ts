/**
 * Plan-mode reference card — companion to the archetype prompt.
 *
 * **Parity contract**: byte-identical port of the in-host
 * `PLAN_MODE_REFERENCE_CARD` at
 * `/Users/lume/repos/openclaw-pr70071-rebase/src/agents/plan-mode/reference-card.ts`
 * (commit `ea04ea52c7`, iter-3 D1 companion to archetype-prompt).
 *
 * # The string is the contract
 *
 * Same as PLAN_ARCHETYPE_PROMPT: bytes hash into the prompt-cache
 * key. Don't paraphrase. If wording needs to change, the in-host
 * source-of-truth changes first; the plugin port mirrors verbatim.
 *
 * # Token budget note
 *
 * The card is INTENTIONALLY compact (~80 lines). Supplements but does
 * NOT replace PLAN_ARCHETYPE_PROMPT (~120 lines, decision-completeness
 * standard). Combined they fit comfortably in the in-mode system
 * prompt without measurable cache impact.
 */
