/**
 * Auto-enable matching for plan mode.
 *
 * Evaluates whether a given model id matches any of the regex patterns
 * configured under `agents.defaults.planMode.autoEnableFor` (or, in
 * plugin terms, `plugins.entries.smarter-claw.config.autoEnableFor`).
 * When a match is found, the runtime caller is expected to flip the
 * session into plan mode at session start (unless the user has already
 ...
 * @param patterns - array of regex pattern strings from the plugin
 *   config (`autoEnableFor`).
 */
